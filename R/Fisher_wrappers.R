...
#'   If provided, built-in pathways are loaded via \code{catfish_load_pathways()}.
...
#' @param out_dir Directory to write CSV when \code{output = TRUE} (default "catfish_fisher").
#'
...
catfish_fisher_pathways <- function(gene_results,
  ...
  if (isTRUE(output)) {
    ...
    out_path <- file.path(out_dir, paste0("catfish_fisher_pathways_", species_tag, ".csv"))
