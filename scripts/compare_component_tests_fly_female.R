# ==============================================================================
# Compare Component Tests from CATFISH Omnibus Results - FLY FEMALE
...
venn_plot <- ggVennDiagram(
  ...
  labs(
    title = "Overlap of Significant Pathways (Fly Female)",
...
quartz()
print(venn_plot)
ggsave("venn_component_tests_fly_female.png", venn_plot,
       width = 8, height = 8, dpi = 300, bg = "white")

cat("\nVenn diagram saved to venn_component_tests_fly_female.png\n")
...
png("upset_component_tests_fly_female.png",
...
cat("UpSet plot saved to upset_component_tests_fly_female.png\n")
...
jaccard_heatmap <- ggplot(
  ...
  labs(title = "Pairwise Jaccard Similarity (Fly Female)",
...
ggsave("jaccard_heatmap_component_tests_fly_female.png", jaccard_heatmap,
       width = 8, height = 8, dpi = 300)

cat("Jaccard heatmap saved to jaccard_heatmap_component_tests_fly_female.png\n")
...
spearman_heatmap <- ggplot(
  ...
  scale_fill_gradient2(low = "red", mid = "white", high = "darkblue",
                       midpoint = 0.5, limits = c(0, 1)) +
  labs(title = "Spearman Rank Correlation (Fly Female)",
...
ggsave("spearman_heatmap_component_tests_fly_female.png", spearman_heatmap,
       width = 8, height = 8, dpi = 300)

cat("Spearman heatmap saved\n")
...
write.csv(top_pathways_combined, "top_pathways_per_method_fly_female.csv",
          row.names = FALSE)
cat("\nTop pathways table saved to top_pathways_per_method_fly_female.csv\n")
...
density_plot <- ggplot(
  ...
  labs(
    title = "P-value Distribution Across Component Tests (Fly Female)",
...
quartz()
print(density_plot)
ggsave("pvalue_density_component_tests_fly_female.png", density_plot,
       width = 8, height = 8, dpi = 300)
...
boxplot_plot <- ggplot(
  ...
  labs(
    title = "P-value Distribution (Fly Female)",
    x = "Method",
...
print(boxplot_plot)
ggsave("pvalue_boxplot_component_tests_fly_female.png", boxplot_plot,
       width = 8, height = 8, dpi = 300)
...
if (requireNamespace("GGally", quietly = TRUE)) {
  ...
  pairs_plot <- ggpairs(
    pval_wide %>% select(-pathway_id) %>%
      mutate(across(everything(), ~-log10(.))),
    title = "Pairwise Comparison of -log10(p) (Fly Female)",
  ...
  ggsave("pairwise_pvalue_scatter_fly_female.png", pairs_plot,
         width = 12, height = 12, dpi = 300)
  cat("Pairwise scatterplot saved to pairwise_pvalue_scatter_fly_female.png\n")
} else {
