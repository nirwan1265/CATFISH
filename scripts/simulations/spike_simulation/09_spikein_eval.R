#!/usr/bin/env Rscript
# =============================================================================
# 09_spikein_eval.R  --  Evaluate staged spike-in recovery and calibration.
#
#   *** UPDATED: now evaluates BOTH the MVN-calibrated omnibus (omni_p_final /
#       omni_p_mvn) AND the analytic omnibus (omni_p_analytic), side by side,
#       on the SAME null sets. This lets us see whether residual inflation is
#       an MVN/tail artifact or an upstream (gene-level) property that the
#       analytic shares. Output columns gain a `pcol` field naming which
#       p-value column each row was computed from.
# =============================================================================
suppressMessages({
  library(data.table)
  library(ggplot2)
...
# calibration for one p-vector, one null-set label, one p-column name
calibration_table <- function(p, label, pcol) {
...
# --- p-value columns to evaluate: MVN-calibrated AND analytic ---------------
mvn_col <- if ("omni_p_final" %in% names(dat)) "omni_p_final" else "omni_p_mvn"
pcols <- intersect(c(mvn_col, "omni_p_analytic"), names(dat))
cat(sprintf("[eval] evaluating p-columns: %s\n", paste(pcols, collapse = ", ")))

# ---- POWER for each p-column ------------------------------------------------
power_all <- rbindlist(lapply(pcols, function(pc) {
  per_sim <- dat[, {
    bh <- p.adjust(get(pc), method = "BH")
    bonf <- p.adjust(get(pc), method = "bonferroni")
    i <- which(pathway_id == spiked)
    data.table(
      spiked_p = if (length(i)) get(pc)[i] else NA_real_,
      spiked_fdr = if (length(i)) bh[i] else NA_real_,
      spiked_bonf = if (length(i)) bonf[i] else NA_real_,
      spiked_rank = if (length(i)) rank(get(pc))[i] else NA_real_
    )
  }, by = perm]
  data.table(
    pcol = pc,
...
fwrite(power_all, file.path(out_dir, "power_summary.csv"))

# keep per_sim for the MVN column for the plot below
per_sim <- dat[, {
  i <- which(pathway_id == spiked)
  data.table(spiked_p = if (length(i)) get(mvn_col)[i] else NA_real_)
}, by = perm]
...
# ---- CALIBRATION: every p-column x every null-set --------------------------
calib <- rbindlist(lapply(pcols, function(pc) {
  rbindlist(lapply(names(null_sets), function(nm) {
    kept <- null_sets[[nm]]
    calibration_table(dat[pathway_id %chin% kept][[pc]], nm, pc)
  }), fill = TRUE)
}), fill = TRUE)
fwrite(calib, file.path(out_dir, "calibration_unspiked.csv"))

# quick side-by-side console print so the MVN-vs-analytic answer is obvious
cat("\n[eval] type-I @0.05 by p-column x null-set:\n")
print(dcast(calib[alpha == 0.05], null_set ~ pcol, value.var = "observed"))
cat("\n")
...
# ---- plot: spiked vs null, using the MVN column (unchanged behavior) --------
...
plot_null_p <- dat[pathway_id %chin% plot_null_paths][[mvn_col]]
...
# subtitle now shows BOTH columns' type-I on the plotted null set
sub_dt <- calib[alpha == 0.05 & null_set == plot_null_set]
subtitle_txt <- paste(sprintf("%s type-I=%.3f", sub_dt$pcol, sub_dt$observed), collapse = " | ")
mvn_power <- power_all[pcol == mvn_col & metric == "FDR<0.05", power]
...
p <- ggplot(plot_dt, aes(val, fill = grp)) +
  ...
  labs(
    ...
    subtitle = sprintf("power(FDR<0.05)=%.2f | %s", mvn_power, subtitle_txt)
  ) +
