#!/usr/bin/env Rscript
# =============================================================================
# 03_magma_catfish.R  --  MAGMA gene analysis for ONE (permutation, chr) using
#                         CATFISH::magma_gene(), mirroring your MAGMA.R exactly
#                         (multi=snp-wise, N_eff = N_TOTAL - n_miss, auto column
#                         detection). The annotation is REUSED from your existing
#                         per-chr .genes.annot (genotype-only, constant), so we
#                         do NOT rebuild snp-loc / re-annotate per permutation.
#
# Usage (from the LSF array wrapper):  Rscript 03_magma_catfish.R <perm> <chr2digit>
# =============================================================================
...
GWAS_SEP        <- ""                               # GEMMA assoc is whitespace-delimited
...
# ---- auto-detect SNP / P / NMISS columns (same logic as MAGMA.R) ------------
find_first_match <- function(choices, available) {
  hit <- choices[choices %in% available]; if (length(hit)) hit[[1]] else NA_character_
}
hdr <- utils::read.table(GWAS_FILE, header = TRUE, sep = GWAS_SEP, nrows = 5,
...
cat(sprintf("[magma] perm=%d chr=%s N_TOTAL=%d cols(SNP=%s,P=%s,NMISS=%s)\n",
            B, CHR, N_TOTAL, RENAME_COLUMNS["SNP"], RENAME_COLUMNS["PVALUE"], RENAME_COLUMNS["NMISS"]))

# ---- gene analysis (reuses existing annotation) -----------------------------
CATFISH::magma_gene(
  bfile          = BFILE_PREFIX,
