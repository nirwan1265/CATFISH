...
# pathway file: same one your real run used (override via PATHWAY_FILE env if needed)
...
if (PATCH_EMP_NULL_LOWER) {
  ...
  assign(".emp_p_null_lower", function(p_null) {
    ...
    # Score each null draw on the same tail-probability scale as the observed
    # component p-value instead of using the old rank/(B+1) transform.
...
# ---- combine this permutation's per-chromosome MAGMA gene outputs -----------
...
# ---- load pathways EXACTLY as the real run (read file, pass pathways=) -------
if (!file.exists(PATHWAY_FILE)) stop("PATHWAY_FILE not found: ", PATHWAY_FILE)
pr <- utils::read.delim(PATHWAY_FILE, header=TRUE, stringsAsFactors=FALSE)
...
omni <- catfish_omni2_pathways(
  ...
  # Component-first MVN calibration remains available as an override, but the
  # direct omnibus calibration (`FALSE`) is the default production setting.
