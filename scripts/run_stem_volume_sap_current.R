...
this_file <- if (length(file_arg)) sub("^--file=", "", file_arg[[1]]) else "scripts/run_stem_volume_sap_current.R"
...
Sys.setenv(
  ...
  PATHWAY_FILE = Sys.getenv(
    "PATHWAY_FILE",
    file.path(repo_dir, "inst/extdata/pathway/sorghumbicolorcyc_pathways.20230103.SORBI")
  ),
  B_PERM = "10000",
  PERM_MODE = "mvn",
  OMNIBUS = "ACAT",
...
sys.source(
  file.path(script_dir, "Usage_Stem_volume_SAP.R"),
  envir = globalenv()
)
