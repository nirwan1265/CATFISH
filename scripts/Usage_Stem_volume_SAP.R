...
TAU_LABEL <- Sys.getenv(
  "TAU_LABEL",
  if (identical(TAU_OPTION, "strict")) "strict_tau" else if (identical(TAU_OPTION, "paper")) "paper_tau_false" else "default_tau"
)
OUT_SUFFIX <- if (nzchar(TAU_LABEL)) paste0("_", TAU_LABEL) else ""

MVN_MARGINAL             <- Sys.getenv("MVN_MARGINAL", "uniform")
MVN_CALIBRATE_COMPONENTS <- get_env_flag("MVN_CALIBRATE_COMPONENTS", FALSE)
...
# Keep legacy sorghum-style filenames too so old plotting scripts remain usable.
legacy_gene_file <- file.path(OUT_DIR, "sorghum_stem_vol_genes_combined.txt")
utils::write.table(
  genes_all,
  legacy_gene_file,
  sep = "\t",
...
cat("Legacy combined genes written to: ", legacy_gene_file, "\n", sep = "")
...
legacy_cor_pairs_file <- file.path(OUT_DIR, "sorghum_stem_vol_gene_cor_pairs.txt")
if (!identical(normalizePath(cor_pairs_file, mustWork = FALSE), normalizePath(legacy_cor_pairs_file, mustWork = FALSE))) {
  file.copy(cor_pairs_file, legacy_cor_pairs_file, overwrite = TRUE)
}

cat("Correlation-pair file written to: ", cor_pairs_file, "\n", sep = "")
cat("Legacy correlation file written to: ", legacy_cor_pairs_file, "\n", sep = "")
...
legacy_results_csv <- file.path(OUT_DIR, "sorghum_stem_vol_catfish_results_MVN.csv")
legacy_b_csv <- file.path(OUT_DIR, paste0("sorghum_stem_vol_CATFISH_B", B_PERM, "_", tail_label, ".csv"))
utils::write.csv(omni_results, legacy_results_csv, row.names = FALSE)
utils::write.csv(omni_results, legacy_b_csv, row.names = FALSE)

cat("CATFISH results written to:\n")
cat("  ", results_csv, "\n", sep = "")
cat("  ", results_rds, "\n\n", sep = "")
cat("Legacy CATFISH CSVs written to:\n")
cat("  ", legacy_results_csv, "\n", sep = "")
cat("  ", legacy_b_csv, "\n\n", sep = "")
