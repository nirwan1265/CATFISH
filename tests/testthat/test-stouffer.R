...
# ============================================================
# Tests for catfish_stoufferZ_pathways
# ============================================================

test_that("catfish_stoufferZ_pathways returns correct columns", {
  ...
  result <- catfish_stoufferZ_pathways(
...
test_that("catfish_stoufferZ_pathways returns correct number of rows", {
  gene_results <- mock_gene_results(10)
  pathways <- list(
    pwy1 = c("gene1", "gene2"),
    pwy2 = c("gene3", "gene4"),
...
test_that("catfish_stoufferZ_pathways handles data.frame pathways", {
  gene_results <- mock_gene_results(10)
...
test_that("catfish_stoufferZ_pathways errors without pathways or species", {
  gene_results <- mock_gene_results(10)

  expect_error(
    catfish_stoufferZ_pathways(gene_results = gene_results),
    "must provide either"
  )
})

test_that("catfish_stoufferZ_pathways errors with both pathways and species", {
  gene_results <- mock_gene_results(10)
  pathways <- list(pwy1 = c("gene1", "gene2"))

  expect_error(
    catfish_stoufferZ_pathways(
      gene_results = gene_results,
      pathways = pathways,
      species = "maize"
    ),
...
test_that("catfish_stoufferZ_pathways validates required columns", {
  ...
  expect_error(
    catfish_stoufferZ_pathways(gene_results = gene_results, pathways = pathways),
    "must contain columns"
  )
})

test_that("catfish_stoufferZ_pathways p-values are in valid range", {
  set.seed(42)
...
test_that("catfish_stoufferZ_pathways is case-insensitive", {
  gene_results <- data.frame(
...
test_that("catfish_stoufferZ_pathways results are sorted by p-value", {
  set.seed(123)
  gene_results <- mock_gene_results(20)
  pathways <- list(
    pwy1 = paste0("gene", 1:5),
...
test_that("catfish_stoufferZ_pathways supports different alternatives", {
  set.seed(42)
  gene_results <- mock_gene_results(10)
  pathways <- list(pwy1 = paste0("gene", 1:5))

  result_greater <- catfish_stoufferZ_pathways(
    gene_results = gene_results,
    pathways = pathways,
    alternative = "greater"
  )

  result_two <- catfish_stoufferZ_pathways(
    gene_results = gene_results,
    pathways = pathways,
    alternative = "two.sided"
  )

  result_less <- catfish_stoufferZ_pathways(
    gene_results = gene_results,
    pathways = pathways,
    alternative = "less"
  )
...
test_that("catfish_stoufferZ_pathways supports weighted analysis", {
  set.seed(42)
  gene_results <- data.frame(
  ...
  result_unweighted <- catfish_stoufferZ_pathways(
    gene_results = gene_results,
    pathways = pathways,
    weight_col = NULL
  )

  result_weighted <- catfish_stoufferZ_pathways(
    gene_results = gene_results,
    pathways = pathways,
    weight_col = "NSNPS"
  )
