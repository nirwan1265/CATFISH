...
# ============================================================
# Tests for catfish_minp_pathways
# ============================================================

test_that("catfish_minp_pathways returns correct columns", {
...
test_that("catfish_minp_pathways returns correct number of rows", {
  gene_results <- mock_gene_results(10)
  pathways <- list(
    pwy1 = c("gene1", "gene2"),
    pwy2 = c("gene3", "gene4"),
...
test_that("catfish_minp_pathways handles data.frame pathways", {
  gene_results <- mock_gene_results(10)
...
test_that("catfish_minp_pathways errors without pathways or species", {
  gene_results <- mock_gene_results(10)

  expect_error(
    catfish_minp_pathways(gene_results = gene_results),
    "must provide either"
  )
})

test_that("catfish_minp_pathways errors with both pathways and species", {
  gene_results <- mock_gene_results(10)
  pathways <- list(pwy1 = c("gene1", "gene2"))

  expect_error(
    catfish_minp_pathways(
      gene_results = gene_results,
      pathways = pathways,
      species = "maize"
    ),
...
test_that("catfish_minp_pathways validates required columns", {
  gene_results <- data.frame(WRONG_COL = c("a", "b"), P = c(0.1, 0.2))
  pathways <- list(pwy1 = c("a", "b"))

  expect_error(
    catfish_minp_pathways(gene_results = gene_results, pathways = pathways),
    "must contain columns"
  )
})

test_that("catfish_minp_pathways minp_stat is actual minimum", {
...
test_that("catfish_minp_pathways p-values are in valid range", {
  set.seed(42)
...
test_that("catfish_minp_pathways is case-insensitive", {
  gene_results <- data.frame(
    GENE = c("GENE1", "GENE2", "GENE3"),
    P = c(0.01, 0.05, 0.1),
    stringsAsFactors = FALSE
...
test_that("catfish_minp_pathways results are sorted by p-value", {
  set.seed(123)
  gene_results <- mock_gene_results(20)
  pathways <- list(
    pwy1 = paste0("gene", 1:5),
...
test_that("catfish_minp_pathways reports method used", {
  gene_results <- mock_gene_results(10)
  pathways <- list(pwy1 = c("gene1", "gene2", "gene3"))
