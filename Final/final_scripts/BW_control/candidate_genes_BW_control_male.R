...
TRAIT          <- "BW_control_male"
CHROMS         <- c("2L", "2R", "3L", "3R", "X")
MAGMA_BASE     <- "/Users/nirwantandukar/Documents/Research/results/CATFISH/MAGMA/BW_control_male"
BASE_DIR       <- "/Users/nirwantandukar/Documents/Research/results/CATFISH/DGRP/BW_control_male"
CATFISH_SUBDIR <- "CATFISH_permutation_B1000000_mvn_GPD"
GWAS_FILE      <- "/Users/nirwantandukar/Documents/Research/results/DGRP/DGRP_chilipeppers/GWAS_BLUEs/BW_control_male.assoc.txt"
...
pick_completed_catfish_csv <- function(base_dir, subdir, trait) {
  ...
  if (!length(hit)) stop("No CATFISH CSV found. Run run_catfish_BW_control_male.R first.", call. = FALSE)
...
pick_completed_gene_table <- function(base_dir, subdir, trait) {
  ...
  if (!file.exists(hit)) stop("No combined gene table found. Run run_catfish_BW_control_male.R first.", call. = FALSE)
  hit
}
