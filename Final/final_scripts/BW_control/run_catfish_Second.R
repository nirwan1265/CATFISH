#!/usr/bin/env Rscript
# CATFISH Pipeline — Second (Drosophila DGRP)
# B = 1,000,000 | perm_mode = "mvn" | GPD | 12 threads

suppressPackageStartupMessages({
  library(CATFISH); library(data.table); library(dplyr)
})

TRAIT      <- "Second"
CHROMS     <- c("2L", "2R", "3L", "3R", "X")
MAGMA_BASE    <- "/Users/nirwantandukar/Documents/Research/results/CATFISH/MAGMA/Second"
OUT_BASE      <- "/Users/nirwantandukar/Documents/Research/results/CATFISH/DGRP/Second"
...
B_PERM <- 1000000L; PERM_MODE <- "mvn"; OMNIBUS <- "ACAT"; N_THREADS <- 12; SEED <- 123
TAU_GRID <- c(0.2, 0.1, 0.05, 0.02, 0.01, 0.005, 0.001)
TAIL_MODE <- "hybrid_gpd"; TAIL_SWITCH_EXCEED <- 10L; TAIL_GPD_K <- 250L
TAIL_MIN_B <- 10000L; TAIL_MIN_TAIL <- 50L; MIN_P <- 1e-30
...
genes_list <- lapply(CHROMS, function(chr) {
  ...
  if (!file.exists(f)) { message("  Skipping: ", chr); return(NULL) }
  x <- read.table(f, header=TRUE, stringsAsFactors=FALSE, check.names=FALSE)
  message("  ", chr, ": ", nrow(x), " genes"); x
})
...
adj <- catfish_adjust_gene_p(gene_results=genes_all, gene_lengths=gene_lengths,
  gene_col="GENE", nsnp_col="NSNPS", p_col="P", z_col="ZSTAT",
  len_gene_col="gene_id", len_col="length")
...
if (!file.exists(cor_pairs_file)) {
  ...
  for (i in seq_along(raw_files)) {
    ...
    tryCatch({
      magma_genesraw_to_cor_pairs_banded(raw_files[[i]], tmp_out, keep_abs_r_ge=0)
      tmp_files <- c(tmp_files, tmp_out); cat("  Done:", chr_name, "\n")
    }, error = function(e) message("  Warning: ", e$message))
  }
  if (length(tmp_files) > 0) {
    fwrite(rbindlist(lapply(tmp_files, fread)), cor_pairs_file, sep="\t")
    file.remove(tmp_files)
  }
} else { cat("Correlation pairs exist:", cor_pairs_file, "\n") }

cat("\nLoading FlyCyc pathways...\n")
fly_pw <- read.delim(PATHWAY_FILE, stringsAsFactors=FALSE)
cat(sprintf("  %d pathways\n\n", length(unique(fly_pw$pathway_id))))

cat("Running CATFISH (B =", B_PERM, ")...\n\n")
omni <- catfish_omni2_pathways(
  gene_results=genes_adj, pathways=fly_pw, gene_col="GENE",
  p_raw_col="P", z_col="ZSTAT", tau_grid=TAU_GRID, omnibus=OMNIBUS,
  B_perm=B_PERM, perm_mode=PERM_MODE, magma_cor_file=cor_pairs_file,
  mvn_marginal="uniform", mvn_calibrate_components=FALSE,
  ...
  min_p=MIN_P, min_genes=2, n_threads=N_THREADS, seed=SEED,
  output=TRUE, out_dir=OUT_DIR)
...
bonf <- 0.05 / nrow(omni)
cat("\n=== RESULTS:", TRAIT, "===\n")
cat("Pathways tested:        ", nrow(omni), "\n")
cat("Bonferroni significant: ", sum(omni[[omni_col]] < bonf), "\n")
cat("FDR < 0.05:             ", sum(p.adjust(omni[[omni_col]], "BH") < 0.05), "\n\n")
top20 <- omni[order(omni[[omni_col]]), ]
print(top20[1:min(20,nrow(top20)), c("pathway_id","pathway_name","n_genes",omni_col)], row.names=FALSE)
cat("\nSaved to:", out_csv, "\n=== DONE:", TRAIT, "===\n")
