...
catfish_file <- file.path(
  base_dir,
  "CATFISH_permutation_B10000_mvn_empirical_strict_tau",
  "Dry_tons_per_acre_CATFISH_ACAT_mvn_B10000_empirical_strict_tau.csv"
)
gene_score_file <- file.path(
  base_dir,
  "candidate_gene_scoring_B10000_empirical_strict_tau",
  "candidate_genes_all_Dry_tons_per_acre_B10000_empirical_strict_tau.csv"
)
...
panel_a_top <- ggplot(combo_df, aes(x = factor(combo_id, levels = combo_id), y = intersection_size)) +
  ...
  labs(title = "Significant Pathway Overlap Across CATFISH Tests", x = NULL, y = "Intersection Size") +
  panel_theme +
  theme(
...
# ------------------------------------------------------------------------------
# Panel B: Omnibus QQ plot with significance categories
# ------------------------------------------------------------------------------
omni_p <- suppressWarnings(as.numeric(catfish_df[[component_cols[["Omnibus"]]]]))
omni_valid <- is.finite(omni_p) & !is.na(omni_p) & omni_p > 0
omni_df <- catfish_df[omni_valid, , drop = FALSE] %>%
  mutate(
    omni_p = omni_p[omni_valid],
    omni_bh = p.adjust(omni_p, method = "BH")
  ) %>%
  arrange(omni_p) %>%
  mutate(
    expected = -log10(ppoints(n())),
    observed = -log10(omni_p),
    sig_class = case_when(
      omni_p < bonf_thresh ~ "Bonferroni < 0.05",
      omni_bh < 0.05 ~ "BH < 0.05",
      TRUE ~ "Not significant"
    ),
    sig_class = factor(sig_class, levels = c("Bonferroni < 0.05", "BH < 0.05", "Not significant"))
  )

panel_b <- ggplot(omni_df, aes(x = expected, y = observed, color = sig_class)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray45", linewidth = 0.5) +
  geom_point(size = 2.1, alpha = 0.9) +
  scale_color_manual(values = c(
    "Bonferroni < 0.05" = "#B54A4A",
    "BH < 0.05" = "#5E92C4",
    "Not significant" = "#A9A9A9"
  ), drop = FALSE) +
  labs(
    title = "Omnibus Calibration and Final Pathway Significance",
    x = expression(Expected ~ -log[10](italic(p))),
    y = expression(Observed ~ -log[10](italic(p))),
    caption = paste0(
      "n = ", nrow(omni_df),
      " pathways | Bonferroni: ", sum(omni_df$omni_p < bonf_thresh, na.rm = TRUE),
      " | BH < 0.05: ", sum(omni_df$omni_bh < 0.05, na.rm = TRUE)
    )
  ) +
  panel_theme +
  theme(
...
panel_c <- ggplot(matrix_long, aes(x = MethodX, y = MethodY, fill = value)) +
  ...
  labs(
    title = "Method Concordance and Complementarity",
    x = NULL,
    y = NULL,
...
panel_d <- ggplot(gene_df, aes(x = log_gwas, y = log_magma, color = evidence, size = point_size)) +
  ...
  labs(
    title = "Integrated Candidate-Gene Prioritization",
...
ggsave(
  filename = file.path(final_fig_dir, "Fig7_B10000_empirical_strict.png"),
  plot = fig7,
  width = 16,
  height = 14,
  dpi = 300,
...
ggsave(
  filename = file.path(final_fig_dir, "Fig7_panelB_B10000_empirical_strict.png"),
  plot = panel_b,
  width = 8.5,
  height = 6.2,
  dpi = 300,
...
message("Fig7 output written to Final/final_main_fig/Fig7_B10000_empirical_strict.png")
