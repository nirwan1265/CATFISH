...
To benchmark cross-sex reproducibility at the locus level, we first compared the highest-ranked genes from the sex-stratified GWAS (Fig. A). Using SNP-to-gene annotation on the lead GWAS signals and retaining the top 50 genes per sex, we observed minimal overlap: only 4 genes were shared between females and males, whereas 46 genes were unique to each sex. We next repeated the comparison after SNP-to-gene aggregation with MAGMA (Fig. B), which integrates information across SNPs within a gene and accounts for LD structure. Gene-level concordance remained limited: 5 genes were shared and 45 were unique to each sex. Thus, whether genes are prioritized directly from individual GWAS hits or after MAGMA aggregation, the leading gene lists are largely sex-specific, consistent with a polygenic setting where small differences in effect size estimates, LD tagging of causal variation, and sampling variability can reorder top-ranked genes across strata.

Because gene-level ranks are sensitive in polygenic traits, we then evaluated cross-sex reproducibility at the level of predefined functional gene sets (pathways), where distributed enrichment is expected to be more stable than single-gene “top hit” lists. We used five component pathway statistics in the CATFISH framework (ACAT, Fisher, adaptive TFisher, minP, and Stouffer) together with the correlation-aware omnibus calibration (Omni_MVN), and summarized results using a rank-based Top-K definition rather than a fixed p/FDR cutoff. This choice is motivated by the modest and uneven pathway-level significance in these data: applying hard thresholds can yield highly variable and sometimes near-empty pathway sets (particularly in males), making apparent overlap largely reflect power rather than concordance. In contrast, Top-K enforces equal list sizes across sexes and methods and therefore compares relative pathway prioritization. We set K = 20 for the main pathway summaries because it is the smallest K at which a clear six-way consensus intersection is present in both sexes; at K = 15, intersections remain partial (male maximum intersection size = 4; female = 5) and no intersection is shared simultaneously by all six methods, whereas at K = 20 a six-way intersection emerges in both sexes (male: 6 pathways shared by all methods; female: 4 pathways shared by all methods). Larger K values (50/100) expand the consensus core but produce denser intersection patterns that are less interpretable in a single main-text panel; accordingly, K-sensitivity is provided separately.

Cross-sex pathway concordance (female vs male Top-20) was strongly dependent on the enrichment statistic (Fig. C). ACAT and Fisher exhibited limited overlap between sexes (1 and 2 shared pathways, respectively), adaptive TFisher recovered a larger shared set (5 shared pathways), and Stouffer and Omni_MVN showed the strongest concordance (7 shared pathways each). In contrast, minP yielded no shared pathways. This divergence is expected from the mechanics of the statistics: minP is effectively “winner-take-all,” being dominated by the single most significant gene within a pathway, so modest sex-specific changes in the identity or strength of the lead gene can shift pathway ranks substantially and eliminate Top-K overlap even when weaker, distributed signal is shared. By comparison, Stouffer (and, in many cases, TFisher) is more sensitive to pathway-wide, distributed enrichment, which is more likely to be reproducible across sexes when many genes contribute small effects.

We next asked whether Omni_MVN’s strong cross-sex concordance could be explained simply by it reproducing one of the component tests. To decompose the omnibus signal, we compared (within each sex) the Top-20 Omni_MVN pathways against the Top-20 pathways from each individual method (Fig. D). Omni_MVN overlapped most strongly with Fisher in females (18/20 shared pathways), with substantial overlap with TFisher and Stouffer (14/20 each), and lower overlap with ACAT (12/20) and minP (10/20). In males, Omni_MVN aligned most closely with Stouffer (18/20) and TFisher (17/20), with moderate overlap with Fisher (10/20) and minP (11/20), and the lowest overlap with ACAT (8/20). Importantly, in every comparison Omni_MVN retained a non-trivial subset of pathways not present in the corresponding component Top-20 list (and vice versa), indicating that the omnibus is not a relabeling of any single statistic but an integrative prioritization that draws from multiple enrichment “modes” (e.g., diffuse signal captured by Stouffer/TFisher and aggregated evidence captured by Fisher).

Finally, the within-sex UpSet summaries provide a structural explanation for why an omnibus approach can be more stable than any single component test (Fig. E–F). In females, the Top-20 lists contain a six-method consensus core of 4 pathways shared by all component tests and Omni_MVN, along with several moderate-sized partial intersections (3–4 pathways) and many singleton intersections (Fig. E). In males, the six-method consensus core is larger (6 pathways), but the remaining overlap structure is also fragmented, with multiple method-specific or partially shared intersections (Fig. F). Together, these patterns show that (i) there is a genuine multi-method “core” signal within each sex, but (ii) a substantial fraction of prioritized pathways depends on the enrichment statistic used—precisely the scenario where an omnibus, correlation-aware synthesis can provide a more reproducible summary.

Overall, the figure supports a coherent narrative: cross-sex concordance is limited at the level of top-ranked genes (Fig. A–B), but becomes more detectable at the pathway level (Fig. C), particularly for statistics sensitive to distributed enrichment (Stouffer and Omni_MVN). Omni_MVN’s behavior is consistent with method integration rather than method replacement (Fig. D), and the UpSet structure demonstrates why integration is helpful—because pathway prioritization contains both a shared consensus core and multiple method-specific components within each sex (Fig. E–F).
