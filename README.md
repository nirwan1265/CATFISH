...
We first examined the top genes from the sex-stratified GWAS to assess SNP-level cross-sex reproducibility (Fig. A). SNP-to-gene annotation on SNPs for the top 50 genes per sex revealed limited overlap. Only four genes were common to both female and males. We employed MAGMA (SNP-to-gene aggregation) (Fig. B) to analyze SNPs inside a gene while considering the LD structure. Overlap at the gene level was also sparse. Only five genes out of 50 were common to both sexes. Consequently, whether genes are prioritized directly from GWAS results or following MAGMA aggregation, the gene lists are predominantly sex-specific, aligning with a polygenic framework where minor effect size estimates, LD tagging of causal variation, and sampling variability can reorder the top-ranked genes across strata.

Gene-level rankings are prone to polygenic signal dispersion, so we evaluated cross-sex repeatability at the pathway level, where dispersed enrichment is expected to be more stable than single-gene lists. Within the CATFISH framework, we aggregated gene-level association evidence to pathways using five gene-set statistics (ACAT, Fisher, adaptive TFisher, minP, and Stouffer) together with a correlation-aware omnibus calibration (Omni_MVN). Pathway-level multiple testing was controlled using Benjamini–Hochberg (BH) FDR as the primary confirmatory criterion. In the sex-stratified Drosophila GWAS, neither gene-level nor pathway-level analyses produced a large set of BH discoveries, consistent with limited power when the sample is split by sex and association statistics are further stabilized by mixed-model correction and MAGMA SNP to gene combination. Under this regime, true effects are expected to be weak and distributed, so few individual genes achieve extreme p-values, and multiple-testing adjustment across thousands of genes (and hundreds of pathways) becomes highly stringent. Moreover, because pathway tests are numerous and correlated (shared genes across pathways and correlated gene statistics), stringent significance cutoffs can yield highly variable and often near-empty sets of significant pathways. Thus, any apparent overlap among “significant” sets primarily reflects statistical power rather than biological concordance. Accordingly, alongside BH-adjusted q-values, we summarize pathway evidence using a rank-based Top-K framework (fixed K across sexes and methods) to enable like-for-like comparisons when FDR-controlled lists are sparse. The minimal K at which both sexes show a clearly defined six-way consensus intersection (five component tests plus Omni_MVN) is K = 20, which we use in the principal pathway summary. Despite limited BH-significant discoveries, Omni_MVN produced nominally small pathway p-values and improved cross-sex concordance in Top-K pathway rankings relative to several single-statistic summaries. This pattern is consistent with weak, distributed signal that becomes more stable when aggregated across genes and integrated across enrichment statistics. However, we interpret these results primarily as support for pathway prioritization and reproducibility under modest power rather than as evidence for a large number of FDR-significant pathway discoveries.
...
We next examined whether the strong cross-sex convergence seen for Omni_MVN could be explained by it effectively reproducing a single component statistic, we decomposed the omnibus signal within each sex by comparing the Top-20 pathways prioritized by Omni_MVN to the Top-20 from each constituent method (Fig. D). In females, Omni_MVN showed its greatest concordance with Fisher (18/20 shared pathways), with moderate overlap to TFisher and Stouffer (14/20 each), and lower overlap to ACAT (12/20) and minP (10/20). In males, Omni_MVN most closely matched Stouffer (18/20) and TFisher (17/20), followed by intermediate overlap with minP (11/20) and Fisher (10/20), and the lowest overlap with ACAT (8/20). Across both sexes, Omni_MVN therefore aligns most strongly with the component statistic that best matches the sex-specific enrichment geometry, while remaining non-identical to every single method. Importantly, Omni_MVN does not behave as a direct duplicate of any individual test. Even when overlap is high (e.g., female Fisher; male Stouffer/TFisher), Omni_MVN retains pathways that are not present in the corresponding component Top-20 list and omits some pathways that are component-specific. Thus, Omni_MVN should be interpreted as an integrative, correlation-aware summary of pathway evidence across enrichment statistics, rather than a reparameterization of one statistic. The within-sex UpSet plots further explain why this integrative summary is more stable than relying on any single component test (Fig. E–F). In females, the Top-20 lists contain a six-method consensus core of 4 pathways jointly identified by all five component tests and Omni_MVN, several moderate partial intersections (3–4 pathways), and many singleton intersections uniquely detected by individual methods (Fig. E). In males, the six-method consensus core is larger (6 pathways), but the remaining overlap structure is more fragmented, with many method-specific or partially shared intersections (Fig. F). This structure reflects the fact that the component statistics emphasize different enrichment architectures: minP is driven by the strongest single-gene signal within a pathway, Fisher and ACAT preferentially reward pathways with broader accumulation of small-to-moderate p-values, and Stouffer/TFisher are sensitive to coordinated shifts in gene-level Z-scores or truncated tails. Consequently, component Top-20 sets tend to share a small “core” where signal is strong enough to be detected regardless of statistic, but diverge outside the core because each test highlights different subsets of pathways.

In this context, Omni_MVN shows the broadest cross-method concordance because it integrates evidence across statistics (while accounting for their dependence) rather than committing to a single enrichment definition. Practically, Omni_MVN tends to retain pathways supported consistently across multiple methods and can also promote pathways that receive strong support from a coherent subset of methods, yielding a pathway prioritization that is less test-dependent and therefore more reproducible than any single component statistic alone.




To test SNP-level cross-sex reproducibility, we examined the top genes from the sex-stratified GWAS (Fig. A). SNP-to-gene annotation of the top 50 genes per sex showed little overlap. Only four genes were shared by men and women. MAGMA (SNP-to-gene aggregation) (Fig. B) was used to analyze gene SNPs given the LD structure. Gene overlap was also low. Only five of 50 genes were shared by both sexes. Whether prioritized directly from GWAS results or after MAGMA aggregation, gene lists are predominantly sex-specific, aligning with a polygenic framework where minor effect size estimates, LD tagging of causal variation, and sampling variability can reorder top-ranked genes across strata.

We tested cross-sex repeatability at the pathway level, where dispersed enrichment is more stable than single-gene lists, because gene-level rankings are prone to polygenic signal dispersion. We aggregated gene-level association evidence to pathways using five gene-set statistics (ACAT, Fisher, adaptive TFisher, minP, and Stouffer) and a correlation-aware omnibus calibration (Omni_MVN) in CATFISH. Benjamini–Hochberg (BH) FDR was the primary confirmatory criterion for pathway-level multiple testing. In the sex-stratified Drosophila GWAS, neither gene-level nor pathway-level analyses produced a large set of BH discoveries, consistent with limited power when the sample is split by sex and association statistics are stabilized by mixed-model correction and MAGMA SNP to gene combination This regime expects true effects to be weak and distributed, so few genes achieve extreme p-values and multiple-testing adjustment across thousands of genes (and hundreds of pathways) becomes strict. Because pathway tests are numerous and correlated (shared genes across pathways and correlated gene statistics), strict significance cutoffs can produce highly variable and often near-empty significant pathways. Thus, “significant” sets overlap mostly due to statistical power, not biological concordance. We summarize pathway evidence using BH-adjusted q-values and a rank-based Top-K framework (fixed K across sexes and methods) for like-for-like comparisons when FDR-controlled lists are sparse. The principal pathway summary uses K = 20 as the minimum K at which both sexes have a clearly defined six-way consensus intersection (five component tests plus Omni_MVN). Omni_MVN outperformed several single-statistic summaries in Top-K pathway rankings with nominally small pathway p-values and improved cross-sex concordance despite few BH-significant discoveries. This pattern suggests a weak, distributed signal that stabilizes when aggregated across genes and enrichment statistics. We interpret these results as support for pathway prioritization and reproducibility under modest power, not as evidence for many FDR-significant pathway discoveries.

The enrichment statistic changed female-male pathway ranking significantly (Fig. C). Stouffer's method and Omni_MVN had the most commanility (7 routes each), while ACAT and Fisher's method had low overlap (1 and 2 pathways). Zero overlap was found by MinP. The pathway's key gene drives minP. Even when weaker, broadly distributed signals are shared between sexes, slight sex-specific lead gene identity or effect size can significantly alter pathway rankings and minimize overlap. Stouffer's method (and often TFisher) is more sensitive to pathway-wide, distributed enrichment, making it more reproducible across sexes when many genes have minor effects.

In order to determine whether Omni_MVN’s strong cross-sex convergence reflected (i) effective duplication of a single component statistic versus (ii) genuine multi-method signal integration and/or increased ranking stability under correlated pathway structure, we decomposed the omnibus signal within each sex by comparing its Top-20 pathways to those of each constituent method (Fig. D). In females, Omni_MVN showed the greatest overlap with Fisher (18/20 shared pathways), followed by TFisher and Stouffer (14/20 each), ACAT (12/20), and minP (10/20). In males, Omni_MVN most closely matched Stouffer (18/20) and TFisher (17/20), with lower overlap to minP (11/20) and Fisher (10/20), and the least overlap to ACAT (8/20). Thus, Omni_MVN aligns most strongly with the component statistic that best matches the sex-specific enrichment pattern, while remaining non-identical to every method. Importantly, its Top-20 list is not a direct replica of any single component list; rather, it reflects an integrative, correlation-aware synthesis across enrichment statistics. Within-sex UpSet plots further explain why this integrative summary is more stable than relying on any single component test (Fig. E–F). In females, the Top-20 results contain a six-method consensus core of 4 pathways detected by all five component statistics and Omni_MVN, several moderate partial intersections (3–4 pathways), and many method-specific singletons (Fig. E). In males, the six-method consensus core is larger (6 pathways), but the remaining overlap structure is more fragmented, with many partially shared or method-specific intersections (Fig. F). This pattern is expected because the component tests emphasize different enrichment architectures: minP is driven by the strongest single-gene signal within a pathway, Fisher and ACAT preferentially reward pathways with broader accumulation of small-to-moderate p-values, and Stouffer/TFisher are sensitive to coordinated shifts in gene-level Z-scores or truncated tails. Overall, the decomposition suggests that Omni_MVN’s cross-sex convergence is driven primarily by correlation-aware integration across enrichment statistics (stabilizing rankings under weak, distributed signal), with additional contribution from the shared-gene correlation structure of the pathway library, rather than by effective duplication of any single component test.
