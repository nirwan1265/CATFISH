...
To ensure that differences in discovery patterns reflect genuine sensitivity rather than miscalibration, we evaluated all component tests and the omnibus under a dependence-preserving global null using MVN resampling (Supp. Fig. 3). The QQ panels compare observed p-value distributions to the expected uniform distribution under the null, and the genomic control statistic λ summarizes global inflation/deflation (λ≈1 expected under correct calibration). In both datasets, ACAT, Fisher, minP, and Stouffer exhibit λ values close to 1, consistent with broadly well-controlled null behavior under the MVN calibration framework. The omnibus is also near-null in both datasets (Arabidopsis slightly conservative with λ<1; fly near 1), which is desirable for a combined procedure intended to provide stable Type I control across heterogeneous pathway architectures.

To verify that differences in discovery patterns reflect genuine power differences rather than miscalibration, we assessed all component tests and the omnibus procedure under a dependence-preserving global null using multivariate normal (MVN) resampling (Supp. Fig. 3). The Q–Q plots compare the empirical p-value distributions to the theoretical uniform distribution implied by the null hypothesis, and the genomic control factor λ provides a scalar summary of global inflation or deflation (with λ≈1 indicating appropriate calibration). In both datasets, ACAT, Fisher’s method, minP, and Stouffer’s method yield λ values close to 1, consistent with well-controlled null behavior under the MVN-based calibration scheme. The omnibus test is likewise close to null in both datasets (slightly conservative in Arabidopsis with λ<1, and near 1 in Drosophila), a desirable property for a combined procedure designed to maintain stable Type I error control across heterogeneous pathway architectures.

The main calibration exception is TFisher in the fly female null diagnostics, which shows a markedly elevated λ (Supp. Fig. 3; TFisher λ ≫ 1), indicating a strong departure from the expected null scaling for that component in that dataset under this configuration. Notably, the empirical Type I error at α=0.05 remains at or below nominal in these diagnostics, which implies that TFisher’s deviation is not well-described as a simple “more false positives at 0.05” phenomenon; rather, it signals a broader shape mismatch of the null distribution for that statistic in this setting (e.g., dependence sensitivity and/or adaptive truncation interacting with the correlation model). Crucially, the omnibus does not inherit this instability: its λ and Type I error remain close to nominal, consistent with the omnibus acting as a stabilizing integrator that reduces reliance on any single component’s idiosyncrasies. In practice, these diagnostics justify two conclusions: (i) the overall CATFISH MVN framework provides near-nominal calibration for most components and for the omnibus, and (ii) TFisher can be unusually sensitive in specific dataset/correlation regimes, reinforcing why the omnibus layer—and explicit null diagnostics—are essential rather than optional.

![MVN null diagnostics support calibrated inference](Figures/SuppFig/SuppFig3.null_calibration_combined.png)
*Supp. Fig. 3 | MVN null diagnostics support calibrated inference.  
