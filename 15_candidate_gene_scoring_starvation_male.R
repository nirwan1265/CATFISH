#!/usr/bin/env Rscript
# =============================================================================
# Candidate Gene Scoring for Drosophila Starvation - Male
# =============================================================================
...
gwas_snps <- fread("/Users/nirwantandukar/Documents/Research/data/DGRP/Starvation_stress/raw_gwas/raw_GWAS_Starvation_stress_male_DGRP.csv")
...
magma_files <- list.files(
  path = "/Users/nirwantandukar/Documents/Research/results/DGRP/MAGMA/Fly_magma_genes_by_chr_male",
  pattern = "\\.genes\\.out$",
  full.names = TRUE
)
...
pathway_results <- fread("catfish_omnibus_results_Fly_male_B10000/omni_acat_mvn.csv")
...
fwrite(scored_genes[, ..output_cols], file.path(out_dir, "starvation_male_candidate_genes_scored.csv"))
fwrite(head(scored_genes, 200)[, ..output_cols], file.path(out_dir, "starvation_male_candidate_genes_top200.csv"))
multi_layer <- scored_genes %>% filter(hit_gwas + hit_magma + hit_pathway >= 2)
fwrite(multi_layer[, ..output_cols], file.path(out_dir, "starvation_male_genes_multi_layer.csv"))
