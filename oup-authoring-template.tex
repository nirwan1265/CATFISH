...
<<<<<<< HEAD
To validate CATFISH under realistic pathway dependence, we used MVN-based simulations that vary (i) signal architecture, (ii) within-pathway LD (gene--gene correlation), and (iii) the amount and quality of correlation information available for calibration. Full simulation details can be placed in Supplementary Methods; here we summarize the main results aligned to the figure blocks.

\subsubsection{Archetype recovery reveals architecture-specific performance and power trade-offs}

We first tested whether distinct pathway signal archetypes can be recovered by the CATFISH component tests. Gene-level $Z$-scores were generated from a correlated MVN model with exchangeable correlation ($\rho=0.20$) and fixed pathway size ($m=20$), with the mean vector $\mu$ encoding five archetypes (SDA, CME, DPS, HDS, SGP). For each archetype (500 replicates), we applied ACAT, Fisher, TFisher, minP, Stouffer, and the omnibus, and summarized performance by power at $\alpha=0.05$, mean rank of the method-specific $p$-value (lower is better), and top-method probability (probability of producing the smallest $p$-value) (Fig.~\ref{fig:blockA_archetype_recovery}).

Fig.~\ref{fig:blockA_archetype_recovery} shows clear architecture specificity. Sparse patterns are preferentially detected by tail-sensitive procedures. Under SDA, minP has the highest power, while ACAT is nearly as powerful and most frequently produces the smallest $p$-value. SGP shows the same qualitative behavior, consistent with single-gene proxy signal being dominated by extreme evidence rather than broad enrichment. In contrast, diffuse enrichment favors aggregate methods: under DPS, Stouffer achieves the best mean-rank performance and is most often the top method, while Fisher shows a small power advantage, consistent with many modest shifts being captured by broad combination rules. HDS exhibits an intermediate pattern, where methods sensitive to both strong drivers and distributed support (TFisher/Fisher) perform well, and the omnibus remains competitive across metrics. CME shows the weakest separation, with several methods performing similarly, suggesting that coordinated-moderate regimes are recoverable but less sharply distinguishable at the current parameterization.

This archetype experiment provides a first notion of “power,” but at a single operating point (fixed simulation strength within each archetype). We therefore ran a second, complementary power study that explicitly sweeps effect size (Supp.~Fig.~\ref{fig:blockF_power}). Here, signal density is the fraction of causal genes in the pathway (dense versus sparse), and directional coherence indicates whether causal effects align in sign (concordant) or split between positive and negative shifts (mixed). We varied per-gene effect strength using a mean shift $\delta$ applied to causal-gene $Z$-scores, and evaluated power over a grid of $\delta$ values for multiple pathway sizes ($m=5,25,50$) under MVN calibration. Supp.~Fig.~\ref{fig:blockF_power} confirms the same qualitative message in a GWAS-like regime: no single component dominates across architectures. Dense concordant signals tend to favor broad aggregation (often Fisher/Stouffer-like behavior), sparse signals favor tail-focused behavior (often minP/ACAT-like behavior), and mixed-direction signals penalize directional aggregation due to cancellation. Across these heterogeneous conditions, the omnibus consistently tracks the upper envelope of the component curves as $\delta$ increases, providing stable power when the true architecture is unknown.

\subsubsection{Null calibration under LD and robustness to missing correlations}

We next evaluated type I error control under the global null in the presence of within-pathway LD. We simulated MVN gene-level $Z$-scores under three LD regimes—LD\_independent ($\rho=0$), LD\_moderate ($\rho=0.3$), and LD\_strong ($\rho=0.7$)—and pathway sizes $m=5,25,50$. For each replicate we computed all component tests and an omnibus, and assessed calibration using genomic control inflation $\lambda$ and empirical type I error at $\alpha=0.05$ (Fig.~\ref{fig:blockB_null_calibration}). In this line+bar display, bars represent type I error and lines represent $\lambda$.

Fig.~\ref{fig:blockB_null_calibration} shows that independence-based analytic $p$-values behave as expected only when LD is negligible. Under LD\_independent, analytic and MVN-calibrated results agree closely across methods and pathway sizes. Under LD\_moderate and especially LD\_strong, several analytic component tests become miscalibrated, producing both inflation and conservatism depending on the method. This is visible as deviations of $\lambda$ from 1 and, more importantly, as type I error rates departing from the nominal 0.05. The key point is that ignoring LD does not simply make everything liberal; rather, it distorts the null in a method-specific way. MVN calibration corrects these distortions: MVN-calibrated component tests and the MVN-calibrated omnibus remain close to nominal behavior across LD regimes and pathway sizes (Fig.~\ref{fig:blockB_null_calibration}).

Real analyses often provide only partial gene--gene correlation information (for example, a subset of pairwise correlations), so we evaluated the robustness of the omnibus statistic to missing correlation data (Supp.~Fig.~\ref{fig:blockC_missing_corr}). Starting from a “true” correlation structure, we randomly removed a fraction of off-diagonal entries and compared (i) an analytic fallback that ignores correlations, (ii) MVN calibration using the true correlation matrix, and (iii) MVN calibration after setting missing entries to zero (zero-imputed MVN). Two results are consistent across $m=5,25,50$. First, the analytic fallback is not a safe default under LD: it shows strong departures in $\lambda$ and inflated type I error. Second, MVN calibration remains substantially more stable under missingness, and the zero-imputed MVN strategy stays closest to nominal over a wide range of missingness fractions. Practically, this supports a conservative implementation in which missing correlations are set to zero and the resulting matrix is projected to the nearest positive-definite correlation matrix before MVN resampling.

\subsubsection{Stress test: omnibus robustness under component misspecification}

To mimic realistic settings where one or more component tests are misspecified, we performed a structured “component breakage” stress test under the global null. For each simulated pathway (across LD regimes and sizes $m=5,25,50$), we designated $k=1,2,3,4$ of the five component tests as broken and averaged results over all choices of broken components. A central design feature is that the omnibus was recalibrated against a \emph{matching broken MVN null}: null replicates were generated under the same LD structure and subjected to the same breakage pattern before constructing the omnibus null distribution (Fig.~\ref{fig:blockD_breakage}).

Fig.~\ref{fig:blockD_breakage} shows that the MVN-recalibrated omnibus is substantially more stable than the individual component tests as breakage increases. Empirical type I error for the omnibus remains close to the nominal 0.05 reference across LD regimes, pathway sizes, and breakage levels, while several individual components drift upward in false positives as more components are broken. This deterioration is most visible for methods such as Stouffer and Fisher under nontrivial LD, and is amplified at larger $m$. The $\lambda$ traces provide complementary information but are not sufficient on their own: some methods can show conservative median behavior ($\lambda<1$) while still having non-nominal tail error. Taken together, these panels show that the combination step is reliable only when paired with an LD-aware (and here, breakage-matched) MVN recalibration of the final omnibus.

\subsubsection{Adaptive calibration and leave-one-out diagnostics}

Finally, we examined two practical diagnostics intended to increase robustness: adaptive component filtering and leave-one-out (LOO) sensitivity analysis (Fig.~\ref{fig:blockE_adaptive_loo}). In the adaptive approach, components are screened using training null simulations and poorly calibrated components are dropped prior to forming the omnibus; we then evaluate the resulting reduced omnibus either analytically or with MVN calibration. Under LD\_moderate and LD\_strong, Analytical and Adaptive+Analytical remain miscalibrated, showing inflated type I error even when median $\lambda$ appears conservative. In contrast, MVN calibration provides the most reliable error control across conditions. Adaptive+MVN can sometimes move $\lambda$ closer to 1, but this does not consistently improve type I error and can occasionally worsen it. Overall, these results support MVN calibration as the default inferential strategy, with adaptive filtering treated as an optional diagnostic rather than a replacement for LD-aware calibration.

The LOO analysis provides a complementary sensitivity check: we recomputed calibration after removing each component in turn (e.g., $-{\rm ACAT}$, $-{\rm Fisher}$, $-{\rm Stouffer}$, $-{\rm TFisher}$, $-{\rm minP}$) and compared these to the full set (“All”). The goal is not to improve power, but to verify that the omnibus calibration is not driven by a single brittle component. In practice, the LOO summaries can be reported compactly (Fig.~\ref{fig:blockE_adaptive_loo}) to show that the MVN-calibrated omnibus remains well behaved across exclusions, while analytic variants are more sensitive to which components are included. This strengthens the interpretation that CATFISH behaves as intended: the omnibus aggregates complementary evidence, and MVN calibration provides the inference layer that stabilizes the result under LD.
...
% =========================
% Main Figures (OUP / Bioinformatics-style)
% =========================

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Figures/MainFig/Fig1.block_a_archetype_recovery.png}
    \caption{\textbf{Archetype recovery for CATFISH component tests.}
    Power and ranking summaries for six pathway tests across five signal archetypes (SDA, CME, DPS, HDS, SGP) under correlated MVN simulations ($m=20$ genes; exchangeable correlation with $\rho=0.20$; 500 replicates per archetype).
    \textbf{A.} Power at $\alpha=0.05$ for each method and archetype (darker indicates higher power).
    \textbf{B.} Mean rank of method-specific $p$-values across replicates (lower is better).
    \textbf{C.} Top-method probability, defined as the probability that a method yields the smallest $p$-value among all methods within a replicate.
    The results show architecture-specific sensitivity: tail-focused tests (minP, ACAT) dominate sparse archetypes (SDA, SGP), whereas aggregate tests (e.g., Stouffer, Fisher) are favored under diffuse enrichment (DPS), and hybrid settings (HDS) show intermediate behavior with strong performance from TFisher/Fisher and competitive performance from the omnibus.}
    \label{fig:blockA_archetype_recovery}
\end{figure*}

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Figures/MainFig/Fig2.block_b_linebar.png}
=======
...
\begin{figure*}[!t]
    ...
    \includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{fig/main/Fig2.block_b_linebar.png}
>>>>>>> af36157bc43e263d3ed5cdab21eaf60827bc7573
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Figures/MainFig/Fig3.block_d_linebar.png}
=======
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{fig/main/Fig3.block_d_linebar.png}
>>>>>>> af36157bc43e263d3ed5cdab21eaf60827bc7573
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Figures/MainFig/Fig4.block_e_panel_ab.png}
=======
\subsubsection{Adaptive calibration and leave-one-out diagnostics}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.5\textheight,keepaspectratio]{fig/main/Fig4.block_e_panel.png}
>>>>>>> af36157bc43e263d3ed5cdab21eaf60827bc7573
...
% =========================
% Supplementary Figures (Appendix / Supplementary Materials)
% =========================

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Figures/SuppFig/SuppFig1_block_f_power.png}
    \caption{\textbf{Power under heterogeneous alternatives (MVN calibration).}
    Statistical power of CATFISH component tests and the omnibus across five signal scenarios (Dense\_Strong, Dense\_Weak, Mixed\_Direction, Sparse\_Moderate, Sparse\_Strong) and pathway sizes ($m=5,25,50$), evaluated over an effect-size grid parameterized by a mean shift $\delta$ applied to causal-gene $Z$-scores.
    All results use MVN-calibrated $p$-values to ensure valid inference under within-pathway LD.
    No single component test dominates across all architectures: dense concordant signals favor broad aggregation, sparse signals favor tail-sensitive methods, and mixed-direction signals penalize directional aggregation due to cancellation. The omnibus consistently tracks the upper envelope of the component curves as $\delta$ increases, indicating robust power when the underlying architecture is unknown.}
    \label{fig:blockF_power}
\end{figure*}

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{Figures/SuppFig/SuppFig2_block_c_panel.png}
    \caption{\textbf{Robustness of omnibus calibration to missing correlations.}
    Omnibus calibration under incomplete LD information, evaluated by randomly removing a fraction of off-diagonal entries from the pathway correlation matrix and comparing three strategies: analytic fallback (ignores correlations), MVN calibration using the true correlation matrix, and MVN calibration after imputing missing correlations to zero.
    \textbf{A.} Genomic control inflation $\lambda$ as a function of fraction missing for pathway sizes $m=5,25,50$ (dashed reference at $\lambda=1$).
    \textbf{B.} Empirical type I error at $\alpha=0.05$ as a function of fraction missing (dashed reference at 0.05).
...
To assess pathway-level organization, we applied out CATFISH pathway analysis to 410 sorghum PlantCyc metabolic pathways (ref). CATFISH evaluated pathway enrichment using five component tests (ACAT, Fisher’s method, adaptive soft TFisher, minP, and Stouffer’s method) and an omnibus test combining them. Statistical inference used MVN resampling, with gene–gene correlation matrices from MAGMA to account for LD. We identified 102 pathways at FDR $< 0.05$, of which 73 remained significant after Bonferroni correction ($p < 0.05/410$). Fig.~\ref{fig:figure4}C shows the top 20 pathways and their $p$-value profiles across component and omnibus tests. The strongest pathway-level signals were detected for gluconeogenesis I (GLUCONEO-PWY), L-glutamate biosynthesis IV (GLUGLNSYN-PWY), L-glutamine degradation I (GLUTAMINDEG-PWY), phospholipases (LIPASYN-PWY), and aerobic respiration I (PWY-3781). Together, these pathways provide a coherent framework for interpreting variation in stem volume. Overall, these results show a clear progression from SNP-level associations to gene-level aggregation and then to pathway-level interpretation under LD-aware calibration (Fig.~\ref{fig:figure4}A--C).

\subsection{Integrated candidate gene analysis identifies high-confidence targets for stem volume and sucrose metabolism}

To prioritize genes supported by multiple modalities, we combined evidence from SNP-level GWAS, MAGMA gene-based tests, and CATFISH pathway enrichment (Fig.~\ref{fig:figure5}). Significance thresholds were: GWAS $-\log_{10}(p) > 7$; MAGMA FDR $< 0.05$; and pathway support is defined as membership in a Bonferroni-significant pathway with $p < 0.05$ (Fig.~\ref{fig:figure5}A). In total, 2016 genes met at least one criterion, and 36 were supported by all three layers, forming the highest-confidence set.

We then tested whether pathway membership is associated with stronger gene-level signals. Genes in significant pathways showed more extreme GWAS and MAGMA statistics than non-pathway genes (Wilcoxon $p = 6.34 \times 10^{-4}$ for GWAS; $p = 7.35 \times 10^{-3}$ for MAGMA; Fig.~\ref{fig:figure5}C), indicating that pathway-level signals preferentially highlight genes with stronger trait-relevant associations rather than random pathway members. The correspondence between SNP- and gene-level association signals is shown in the GWAS versus MAGMA scatter plot (Fig.~\ref{fig:figure5}D). The 36 genes supported by all three analytic layers cluster in the upper-right quadrant (red), indicating strong single-variant and aggregated gene-level association. This cross-layer concordance suggests a robust biological signal rather than method-specific artifacts.

These 36 triple-supported genes form the primary set for downstream functional interpretation and validation (Table~\ref{tab:top_genes}). Many participate in pathways directly involved in biomass accumulation, including carbon allocation, nitrogen metabolism, membrane biogenesis, and cellular energy production. The top candidate, \textit{SORBI\_3009G231100} (GWAS rank 2; MAGMA rank 15), is annotated to the CMP-3-deoxy-D-\textit{manno}-octulosonate biosynthesis pathway (PWY-1269), which contributes to cell wall and other polysaccharide processes. Variation affecting cell wall synthesis and remodeling is a plausible determinant of stem volume via effects on radial growth, secondary thickening, and mechanical reinforcement.
