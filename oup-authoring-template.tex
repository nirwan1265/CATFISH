...
We first performed genome-wide association studies (GWAS) for stem volume in the sorghum association panel (SAP). The SNP-level scan identified multiple genome-wide signals and a prominent peak on chromosome 9 (Fig.~\ref{fig:figure4}A). Using a genome-wide threshold of $-\log_{10}(p) > 7$, we mapped significant SNPs to 423 genes. Most of the strongest evidence was concentrated in a single interval on chromosome 9. Specifically, 34 of the 36 top candidate genes localized to a $\sim$2.8~Mb region spanning 56.1--58.9~Mb, consistent with a major QTL for stem volume.

We then aggregated SNP associations to gene-level statistics using MAGMA to account for linkage disequilibrium (LD). We tested 33,967 annotated sorghum genes and assigned SNPs to genes using gene boundaries with $\pm$25~kb flanking windows. After Benjamini--Hochberg false discovery rate correction, 861 genes were significant at FDR $<0.05$ (Fig.~\ref{fig:figure4}B). The strongest gene-level association was observed for \textit{SORBI\_3009G206700} with $-\log_{10}(p)=14.64$. Gene-level signals also clustered on chromosome 9, mirroring the SNP-level peak and supporting a concentrated locus with multiple associated genes.

Finally, we tested whether associated genes aggregate into coherent biological pathways using CATFISH on 410 sorghum PlantCyc metabolic pathways. CATFISH evaluates enrichment using five complementary component tests (ACAT, Fisher, adaptive soft TFisher, minP, and Stouffer) and an omnibus procedure that integrates these signals. Inference was calibrated using multivariate normal (MVN) resampling with gene--gene correlations estimated by MAGMA, which accounts for LD-induced dependence among genes within each pathway. This analysis identified 102 pathways at FDR $<0.05$ and 73 pathways after Bonferroni correction ($p < 0.05/410$). Fig.~\ref{fig:figure4}C shows the top 20 pathways and their method-specific $p$-value patterns across the component tests and the omnibus.

The strongest signals included gluconeogenesis I (GLUCONEO-PWY), L-glutamate biosynthesis IV (GLUGLNSYN-PWY), L-glutamine degradation I (GLUTAMINDEG-PWY), phospholipases (LIPASYN-PWY), and aerobic respiration I (PWY-3781). These pathways support a coherent interpretation for stem volume variation. Stem growth requires coordinated carbon allocation, nitrogen assimilation, membrane biogenesis, and ATP production. The enrichment of gluconeogenesis, amino acid metabolism, phospholipid-related processes, and respiration is consistent with these metabolic demands.

Across pathways, the omnibus statistic provided stable detection when component tests differed in sensitivity. Some pathways showed broadly consistent evidence across all components, indicating robust enrichment. Others exhibited method-specific signatures that were retained by the omnibus integration. Together, these results illustrate a consistent progression from SNP-level associations to gene-level aggregation and pathway-level interpretation under LD-aware calibration (Fig.~\ref{fig:figure4}A--C).


\subsection{Integrated candidate gene analysis identifies high-confidence targets}

To prioritize candidate genes supported by multiple analytical layers, we integrated evidence from SNP-level GWAS, MAGMA gene-based testing, and CATFISH pathway enrichment (Fig.~\ref{fig:figure5}). We used the following significance cutoffs for each layer. GWAS required $-\log_{10}(p) > 7$. MAGMA required FDR $<0.05$. Pathway support required membership in a Bonferroni-significant pathway with $p<0.05$.

The UpSet plot summarizes overlap among these evidence layers (Fig.~\ref{fig:figure5}A). Across all analyses, 2{,}016 genes met at least one criterion. A set of 36 genes appeared in all three layers, representing the highest-confidence candidates. Additional genes showed pairwise overlap between layers, including overlap between GWAS and MAGMA, between MAGMA and pathway-supported genes, and a smaller overlap between GWAS and pathway-supported genes (Fig.~\ref{fig:figure5}A,B). The bar summary highlights that pathway-only genes form the largest single-evidence category (Fig.~\ref{fig:figure5}B), consistent with pathway enrichment capturing broader sets of functionally related genes that may not reach single-gene significance.

We next asked whether pathway membership enriches for stronger association signals at the gene level. Density comparisons showed that genes belonging to significant pathways have more extreme association evidence than non-pathway genes in both the GWAS layer and the MAGMA layer (Wilcoxon $p = 6.34 \times 10^{-4}$ for GWAS and $p = 7.35 \times 10^{-3}$ for MAGMA; Fig.~\ref{fig:figure5}C). This supports the interpretation that pathway calls preferentially select genes with stronger trait-relevant signals rather than reflecting random pathway membership.

The relationship between SNP-based and gene-based evidence is shown by the GWAS versus MAGMA scatter plot (Fig.~\ref{fig:figure5}D). The 36 triple-supported genes cluster toward the upper-right region of the plot, indicating that they tend to carry both strong SNP-level support and strong aggregated gene-level support. This concordance across layers is consistent with genuine biological signal rather than method-specific artifacts.

The 36 genes supported by all three layers constitute the primary set for downstream interpretation and validation (Table~\ref{tab:top_genes}). Many of these candidates map to pathways directly aligned with biomass accumulation, including carbon allocation, nitrogen metabolism, membrane biogenesis, and energy production. The top-ranked candidate, \textit{SORBI\_3009G231100} (GWAS rank 2; MAGMA rank 15), maps to CMP-3-deoxy-D-\textit{manno}-octulosonate biosynthesis (PWY-1269), which contributes to cell wall and polysaccharide-related processes. Variation affecting cell wall synthesis and remodeling is a plausible determinant of stem volume through effects on structural growth and mechanical properties.

Several candidates connect to nitrogen assimilation and amino acid metabolism. \textit{SORBI\_3009G225700} (GWAS rank 4; MAGMA rank 31) appears in multiple enriched pathways, including L-glutamate biosynthesis IV (GLUGLNSYN-PWY) and L-glutamine degradation I (GLUTAMINDEG-PWY). These pathways support protein synthesis and cellular growth processes that scale with stem expansion. Additional candidates connect to lipid and membrane processes, consistent with the requirement for membrane expansion during rapid growth. For example, \textit{SORBI\_3009G225100} (GWAS rank 20; MAGMA rank 50) is linked to phospholipid biosynthesis pathways, and \textit{SORBI\_3009G240600} and \textit{SORBI\_3009G240500} map to phospholipase and fatty acid-related functions.

Energy metabolism also emerges as a central theme. Aerobic respiration I (PWY-3781) is among the most gene-rich enriched pathways, and multiple high-confidence candidates participate in this pathway. This is consistent with the energetic cost of biomass accumulation and the requirement for sustained ATP production during stem growth. We also highlight \textit{SORBI\_3009G240700} (GWAS rank 76; MAGMA rank 235), which connects gluconeogenesis I (ranked as the top pathway), the glyoxylate cycle, and citrate degradation. These pathways jointly support carbon redistribution and provision of biosynthetic precursors, which can contribute to structural growth and cell wall formation.

Finally, genomic location reinforces the integrative signal. Most triple-supported candidates localize to the major chromosome 9 interval identified by GWAS, consistent with either a single major-effect locus in extensive LD or multiple linked causal variants within the region. Two exceptions, \textit{SORBI\_3002G379200} and \textit{SORBI\_3002G379100} on chromosome 2, map to starch-related biosynthetic pathways, suggesting an additional carbon storage or allocation component outside the main chromosome 9 peak. Together, these integrated results define a focused set of high-confidence candidates and provide a coherent functional narrative for stem volume variation in sorghum.
