...
We performed GWAS for stem volume in the sorghum association panel (SAP). SNP-level analysis identified multiple genome-wide significant signals, including a strong peak on chromosome 9 (Fig.~\ref{fig:figure4}A). Using a significance threshold of $-\log_{10}(p) > 7$, we mapped significant SNPs to 423 genes. Most top associations were concentrated within a single chromosome 9 interval: 34 of the 36 leading candidate genes fell in a $\sim$2.8~Mb region (56.1–58.9~Mb), consistent with a major stem volume QTL.

To account for linkage disequilibrium (LD) and obtain gene-level statistics, we aggregated SNP-level results with MAGMA. We tested 33,967 annotated sorghum genes, assigning SNPs to genes with $\pm$25~kb flanks. After Benjamini–Hochberg FDR correction, 861 genes were significant at FDR $<0.05$ (Fig.~\ref{fig:figure4}B). The top gene was \textit{SORBI\_3009G206700} with $-\log_{10}(p)=14.64$. Gene-level signals were likewise enriched on chromosome 9, mirroring the SNP-level peak and indicating a concentrated locus with multiple associated genes.

To test for convergence on shared biological processes, we conducted pathway analyses with CATFISH on 410 sorghum PlantCyc metabolic pathways. CATFISH evaluates pathway enrichment using five component tests (ACAT, Fisher’s method, adaptive soft TFisher, minP, Stouffer’s method) combined by an omnibus procedure. Statistical inference used multivariate normal resampling, with gene–gene correlations from MAGMA to account for LD among genes in each pathway. We identified 102 pathways at FDR $<0.05$ and 73 that remained significant after Bonferroni correction ($p < 0.05/410$). Fig.~\ref{fig:figure4}C shows the top 20 pathways and their method-specific $p$-values and omnibus statistics.

The strongest pathway-level signals were gluconeogenesis I (GLUCONEO-PWY), L-glutamate biosynthesis IV (GLUGLNSYN-PWY), L-glutamine degradation I (GLUTAMINDEG-PWY), phospholipases (LIPASYN-PWY), and aerobic respiration I (PWY-3781). Together, they provide a coherent framework for interpreting stem volume variation, as radial and longitudinal stem growth require coordinated carbon allocation, nitrogen assimilation, membrane biogenesis, and ATP production. Thus, enrichment of gluconeogenic, amino acid metabolic, phospholipid-related, and respiratory pathways aligns with the core metabolic demands of secondary growth.

The omnibus statistic gave stable pathway detection even when individual component tests varied in sensitivity. Some pathways showed consistent evidence across all methods, indicating robust enrichment, whereas others showed method-specific signals that were still captured by omnibus integration. Overall, these results trace a coherent progression from SNP-level associations to gene-level aggregation and pathway-level interpretation under LD-aware calibration (Fig.~\ref{fig:figure4}A--C).


\subsection{Integrated candidate gene analysis identifies high-confidence targets}

To prioritize genes supported by multiple modalities, we integrated evidence from SNP-level GWAS, MAGMA gene-level tests, and CATFISH pathway enrichment (Fig.~\ref{fig:figure5}). Significance thresholds were: GWAS SNPs with $-\log_{10}(p) > 7$; MAGMA genes with FDR $< 0.05$; and pathways passing Bonferroni correction at $p < 0.05$, with genes considered supported if they belonged to such pathways.

The UpSet plot in Fig.~\ref{fig:figure5}A summarizes overlaps among these three evidence layers. In total, 2{,}016 genes met at least one criterion, and 36 were supported by all three, representing our highest-confidence candidates. Additional genes showed pairwise support (GWAS+MAGMA, MAGMA+pathway, or GWAS+pathway; Fig.~\ref{fig:figure5}A,B). The bar plot in Fig.~\ref{fig:figure5}B shows that genes supported only by pathway enrichment form the largest single-evidence group, consistent with pathway analyses capturing broader sets of functionally related genes that may not be significant in single-gene tests.

We next tested whether pathway membership is associated with stronger gene-level association signals. Density comparisons showed that genes in Bonferroni-significant pathways have more extreme association statistics than non-pathway genes in both GWAS and MAGMA (Wilcoxon rank-sum: $p = 6.34 \times 10^{-4}$ for GWAS; $p = 7.35 \times 10^{-3}$ for MAGMA; Fig.~\ref{fig:figure5}C). Thus, pathway-based prioritization preferentially selects genes with stronger trait-relevant signals rather than randomly including annotated genes.

The correspondence between SNP- and gene-level signals is shown in the GWAS–MAGMA scatter plot (Fig.~\ref{fig:figure5}D). The 36 triple-supported genes cluster in the upper-right region, indicating consistently strong single-variant and aggregated gene-level evidence. This cross-layer concordance supports a genuine biological signal rather than method-specific artifacts.

The 36 genes supported by all three analytical layers form the primary candidate set for downstream functional analysis (Table~\ref{tab:top_genes}). Many map to pathways implicated in biomass accumulation, including carbon partitioning, nitrogen metabolism, membrane biogenesis, and energy production. The top candidate, \textit{SORBI\_3009G231100} (GWAS rank 2; MAGMA rank 15), participates in CMP-3-deoxy-D-\textit{manno}-octulosonate biosynthesis (PWY-1269), a cell wall and polysaccharide-related pathway. Genetic variation in cell wall synthesis and remodeling may therefore contribute to stem volume differences via effects on structural growth and mechanics.

Several candidates are involved in nitrogen assimilation and amino acid metabolism. For instance, \textit{SORBI\_3009G225700} (GWAS rank 4; MAGMA rank 31) participates in enriched pathways including L-glutamate biosynthesis IV (GLUGLNSYN-PWY) and L-glutamine degradation I (GLUTAMINDEG-PWY), which support protein synthesis and cellular growth expected to scale with stem expansion. Additional candidates relate to lipid metabolism and membrane functions, consistent with increased membrane demand during rapid growth. \textit{SORBI\_3009G225100} (GWAS rank 20; MAGMA rank 50) is associated with phospholipid biosynthesis, whereas \textit{SORBI\_3009G240600} and \textit{SORBI\_3009G240500} are annotated to phospholipase activity and fatty acid-related processes.

Energy metabolism emerges as a central organizing principle. Aerobic respiration I (PWY-3781) is one of the most gene-dense enriched pathways, with multiple high-confidence candidate genes annotated to it. This aligns with the high energetic demands of biomass accumulation and the need for sustained ATP production during stem elongation and secondary growth. We also highlight \textit{SORBI\_3009G240700} (GWAS rank 76; MAGMA rank 235), which functionally links gluconeogenesis I (the top-ranked pathway), the glyoxylate cycle, and citrate degradation. Together, these pathways support carbon remobilization and biosynthetic precursor production, potentially sustaining structural growth and cell wall biogenesis.

Genomic context further reinforces this signal. Most triple-supported candidate genes cluster within the major chromosome 9 interval identified by GWAS, consistent with either a single major-effect locus in strong linkage disequilibrium or several closely linked causal variants. Two exceptions, \textit{SORBI\_3002G379200} and \textit{SORBI\_3002G379100} on chromosome 2, are associated with starch biosynthesis pathways, suggesting an additional component of carbon storage or allocation independent of the primary chromosome 9 signal. Overall, these integrative analyses define a focused subset of high-confidence candidate genes and a coherent functional framework for understanding natural variation in sorghum stem volume.
