...
To prioritize genes supported by multiple modalities, we combined evidence from SNP-level GWAS, MAGMA gene-based tests, and CATFISH pathway enrichment (Fig.\~\ref\{fig:figure5\}). Significance thresholds were: GWAS \$-\textbackslash{}log\_{10\}(p) > 7\$; MAGMA FDR \$< 0.05\$; and pathway support is defined as membership in a Bonferroni-significant pathway with \$p < 0.05\$ (Fig.\~\ref\{fig:figure5\}A). In total, 2016 genes met at least one criterion, and 36 were supported by all three layers, forming the highest-confidence set.

We then tested whether pathway membership is associated with stronger gene-level signals. Genes in significant pathways showed more extreme GWAS and MAGMA statistics than non-pathway genes (Wilcoxon \$p = 6.34 \textbackslash{}times 10\^{-4\}$ for GWAS; \$p = 7.35 \textbackslash{}times 10\^{-3\}$ for MAGMA; Fig.\~\ref\{fig:figure5\}C), indicating that pathway-level signals preferentially highlight genes with stronger trait-relevant associations rather than random pathway members. The correspondence between SNP- and gene-level association signals is shown in the GWAS versus MAGMA scatter plot (Fig.\~\ref\{fig:figure5\}D). The 36 genes supported by all three analytic layers cluster in the upper-right quadrant (red), indicating strong single-variant and aggregated gene-level association. This cross-layer concordance suggests a robust biological signal rather than method-specific artifacts.

These 36 triple-supported genes form the primary set for downstream functional interpretation and validation (Table\~\ref\{tab:top\_genes\}). Many participate in pathways directly involved in biomass accumulation, including carbon allocation, nitrogen metabolism, membrane biogenesis, and cellular energy production. The top candidate, \textbackslash{}textit\{SORBI\textbackslash{}_3009G231100\} (GWAS rank 2; MAGMA rank 15), is annotated to the CMP-3-deoxy-D-\textbackslash{}textit\{manno\}-octulosonate biosynthesis pathway (PWY-1269), which contributes to cell wall and other polysaccharide processes. Variation affecting cell wall synthesis and remodeling is a plausible determinant of stem volume via effects on radial growth, secondary thickening, and mechanical reinforcement.
